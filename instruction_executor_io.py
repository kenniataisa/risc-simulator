class InstructionExecutor:
    ...
    def _execute_jmp(self, instruction):
        ...
        if signed_imm == 0:
            print("Instrução: NOP (JMP #0)")
            self._simulator.print_final_state() 
    ...
    def _execute_jcond(self, instruction):
        ...
        # JLT (Jump if less than): Salta quando a flag zero não está ativa e a carry está 
            ...
            return True #Retorna para evitar a lógica de impressão abaixo
    ...
    def _execute_str(self, instruction):
        ...
        rn = (instruction >> 8) & 0xF # CORREÇÃO: Rn (fonte do dado) vem dos bits 11-8
        ...
        # --- Início da Lógica de E/S de Saída ---
        if memory_address == 0xF002: # CHAR_OUT
            char_to_print = chr(data_to_store & 0xFF)
            print(f"Instrução: STR R{rn}, [R{rm}] -> SAÍDA CHAR: '{char_to_print}'")
            if self._simulator.output_file_handler:
                self._simulator.output_file_handler.write(char_to_print)
<<<<<<< HEAD
                self._simulator.output_file_handler.flush()
=======
>>>>>>> 232a90c (feat: implementar E/S de arquivo para LDR e STR)
            return True

        elif memory_address == 0xF003: # INT_OUT
            signed_data = self.sign_extend(data_to_store, 16)
            print(f"Instrução: STR R{rn}, [R{rm}] -> SAÍDA INT: {signed_data}")
            if self._simulator.output_file_handler:
                self._simulator.output_file_handler.write(str(signed_data) + '\n')
<<<<<<< HEAD
                self._simulator.output_file_handler.flush()
=======
>>>>>>> 232a90c (feat: implementar E/S de arquivo para LDR e STR)
    ...
    def _execute_ldr(self, instruction):
        ...
        Rd = MEM[Rm] (Opcode: 0010)
         Carrega um valor da memória OU de um dispositivo de entrada mapeado em memória.
        """
        rd = (instruction >> 8) & 0xF
        rm = (instruction >> 4) & 0xF
        ...
        memory_address = self.registers[rm]
         ...
         # --- Início da Lógica de E/S de Entrada ---
        if memory_address == 0xF000: # CHAR_IN
            try:
                char_read = next(self._simulator.input_char_iterator)
                loaded_data = ord(char_read)
                print(f"Instrução: LDR R{rd}, [R{rm}] -> ENTRADA CHAR: '{char_read}'")
            except StopIteration:
                loaded_data = 0 # Fim do arquivo
                print(f"Instrução: LDR R{rd}, [R{rm}] -> ENTRADA CHAR: Fim de arquivo (EOF)")
            self.registers[rd] = loaded_data
            return True

        elif memory_address == 0xF001: # INT_IN
            try:
                line_read = next(self._simulator.input_line_iterator)
                loaded_data = int(line_read.strip()) & 0xFFFF
                print(f"Instrução: LDR R{rd}, [R{rm}] -> ENTRADA INT: {loaded_data}")
            except (StopIteration, ValueError):
                loaded_data = 0 # Fim do arquivo ou linha inválida
                print(f"Instrução: LDR R{rd}, [R{rm}] -> ENTRADA INT: Fim de arquivo ou erro de formato")
            self.registers[rd] = loaded_data
            return True
